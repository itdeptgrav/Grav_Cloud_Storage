...
const fileObjectSchema = new mongoose.Schema(
  {
    ...
    // Provenance. An upload comes from EITHER an API key (machine) OR a human
    // dashboard session; uploadedByLabel is a denormalised display string so the
    // file table never has to join to render the "Uploaded By" column.
    ...
    uploadedByLabel: { type: String, default: "" },
...
fileObjectSchema.methods.toMeta = function toMeta() {
  return {
    ...
    status: this.status,
    uploadedByLabel: this.uploadedByLabel,
