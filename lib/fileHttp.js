...
import { trackTransfer } from "@/lib/projectGuard";
...
// onClosed (internal) fires once the file handle itself is closed.
function countingResponse(nodeStream, { status, headers, onServed, onClosed }) {
  let n = 0;
  let done = false;
  const finish = () => {
    if (done) return;
  ...
  counter.on("close", () => {
    finish();
    // pipe() never destroys its source: when the client goes away mid-download
    // the file would otherwise stay open (on Windows that also blocks deleting
    // its folder).
    if (!nodeStream.destroyed) nodeStream.destroy();
  });
  if (onClosed) nodeStream.once("close", onClosed);
...
export async function serveFile(file, request, { attachment = false, onServed, extraHeaders } = {}) {
  ...
  // A permanent deletion of the project cuts this download off (and waits for
  // the file handle to close); refused outright once the deletion has started.
  const untrack = trackTransfer(file.projectId, () => nodeStream.destroy(Object.assign(new Error("project deleted"), { code: "PROJECT_DELETING" })), "download");
  if (!untrack) {
    nodeStream.destroy();
    return { missing: true };
  }
  return { response: countingResponse(nodeStream, { status, headers, onServed, onClosed: untrack }) };
}
