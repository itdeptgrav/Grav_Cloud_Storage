...
// Upload TELEMETRY only (historical counters). The authoritative storage
// increment (currentStorageBytes/fileCount) is done atomically + quota-gated by
// quotaService.commitStorage — NOT here — so the two never double-count.
export function recordUploadTelemetry(project, key, bytes) {
  Project.updateOne({ _id: project._id }, { $inc: { "counters.uploads": 1, "counters.bytesUp": bytes } }).catch(() => {});
  if (key) ApiKey.updateOne({ _id: key._id }, { $inc: { "counters.uploads": 1, "counters.bytesUp": bytes } }).catch(() => {});
}
...
export function recordDownload(project, key, fileObjectId, bytesServed) {
  ...
  // key is null for dashboard (human-session) reads.
  if (key) ApiKey.updateOne({ _id: key._id }, { $inc: { "counters.downloads": 1, "counters.bytesDown": bytes } }).catch(() => {});
