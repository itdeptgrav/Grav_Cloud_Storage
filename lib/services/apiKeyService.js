...
export async function createApiKey(project, { name, env = "live", scopes, createdByUserId }) {
  ...
  // Inside the project's write fence: no key is minted for a project whose
  // permanent deletion has started (lib/projectGuard).
  ...
  if (run.fenced) throw Object.assign(new Error(PROJECT_DELETING_MESSAGE), { code: "PROJECT_DELETING" });

  return { record: run.value, rawKey };
}
