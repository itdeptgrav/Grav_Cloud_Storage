...
async function opBegin(request, actor) {
  ...
  // Inside the project's write fence: a permanent deletion that has started
  // refuses this, and one that starts meanwhile waits for it (lib/projectGuard).
  const made = await withProjectWrite(project._id, () => createSession(actor, body, { uploadId, size, filename, expected, chunkSize, expiresAt }));
  if (made.fenced) {
    releaseHeldSlot(uploadId);
    return fail("PROJECT_DELETING", PROJECT_DELETING_MESSAGE);
  }
  return made.value;
}

async function createSession(actor, body, { uploadId, size, filename, expected, chunkSize, expiresAt }) {
  const { project } = actor;
...
/**
 * Permanent project deletion: stop and remove EVERY upload of a project.
 * Chunks still streaming are aborted through the same path a retry uses to take
 * over an orphaned attempt (their file handle is closed; nothing more is
 * written), live sessions are closed (temp file deleted, upload slot freed),
 * and the temp file of every session the project ever had is removed and
 * checked gone. Session records are left for the caller to delete. Throws if a
 * temp file cannot be removed.
 */
export async function closeProjectUploads(projectId, { waitMs = TAKEOVER_WAIT_MS } = {}) {
  await connectDB();
  const provider = getStorageProvider();
  const all = await UploadSession.find({ projectId });
  // What is on disk before anything is touched — the count the caller reports.
  const onDisk = new Map();
  for (const s of all) {
    const st = await provider.statChunkTemp(s.tmpObjId);
    if (st) onDisk.set(s.tmpObjId, st.bytes);
  }

  const live = all.filter((s) => s.status === "active" || s.status === "completing");
  const waits = [];
  let chunksAborted = 0;
  for (const s of live) {
    const running = inflight.get(s.uploadId);
    if (running) {
      running.abort.abort();
      waits.push(running.done);
      chunksAborted++;
    }
  }
  if (waits.length) await Promise.race([Promise.all(waits), new Promise((r) => setTimeout(r, waitMs))]);
  for (const s of live) await closeSession(s, "aborted", "PROJECT_DELETED");

  for (const s of all) {
    releaseHeldSlot(s.uploadId);
    await provider.removeChunkTemp(s.tmpObjId); // throws unless removed (or already gone)
    if (await provider.statChunkTemp(s.tmpObjId)) {
      throw Object.assign(new Error("An upload temp file could not be removed."), { code: "EBUSY" });
    }
  }
  let tempBytes = 0;
  for (const b of onDisk.values()) tempBytes += b;
  return { sessions: all.length, closed: live.length, chunksAborted, tempFiles: onDisk.size, tempBytes };
}
