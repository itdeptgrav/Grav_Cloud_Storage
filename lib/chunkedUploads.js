...
async function opBegin(request, actor) {
  ...
  // Chunk size: the server's value, or a SMALLER one the client asks for.
  let chunkSize = config.chunkSizeBytes;
  ...
  return ok(s.toPublic(), { status: 201 });
}
...
async function opAppend(request, actor, url) {
  ...
  const { session: s, error } = await loadOwned(uploadId, actor);
  if (error) {
    cancelBody(request);
    return error;
  }
  ...
  try {
    ...
    try {
      await pipeline(Readable.fromWeb(request.body), meter, provider.openChunkWrite(s.tmpObjId, offset));
    } catch (e) {
      ...
      return fail("CHUNK_INTERRUPTED", `Chunk ${index} did not arrive completely. Retry it.`);
    }

    if (received !== len) {
      await provider.truncateChunkTemp(s.tmpObjId, offset).catch(() => {});
      return fail("CHUNK_SIZE_MISMATCH", `Chunk ${index} must be exactly ${len} bytes (got ${received}).`, { details: { expectedBytes: len } });
    }
