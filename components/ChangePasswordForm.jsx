...
export default function ChangePasswordForm({ onSuccess }) {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      await api.post("/api/auth/change-password", { currentPassword: cur, newPassword: pw, confirmPassword: confirm });
  ...
  return (
    <form onSubmit={submit}>
      ...
      <Field label="New password"><Input type="password" value={pw} onChange={(e) => setPw(e.target.value)} required autoComplete="new-password" /></Field>
      ...
      <Button variant="primary" loading={busy}>Change password</Button>
    </form>
  );
}
